...
import 'package:bachelor_app/UI/pages/map_per_device.dart';
    ...
    return Padding(
      ...
      child: Column(
        children: <Widget>[
          Row(
            children: <Widget>[
              Expanded(
                child: Padding(
                  padding:
                      EdgeInsets.only(left: 0, top: 0, right: 80, bottom: 20),
                  child: Container(
          ...
          FutureBuilder(
              future:
                  DatabaseService(widget.gid, selectedMission ?? "Mission 1")
                      .datas,
              builder: (context, snapshot) {
                if (!snapshot.hasData) return const Text("Loading....");
                Map dataset = new Map<String, dynamic>.from(snapshot.data);
                return Column(children: <Widget>[
                  MapPerDevice(dataset: dataset),
                  OneChart(dataType: dataType, dataset: dataset),
                ]);
              }),
          SizedBox(height: 20),
          Row(
